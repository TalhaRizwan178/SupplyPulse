      ...
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:${T.bg};border-radius:16px;overflow:hidden;border:1px solid ${T.line};">
        ...
        <tr>
          <td style="background:${T.bg2};padding:28px 32px 24px;text-align:center;border-bottom:1px solid ${T.line};">
            ...
            ${hasLogo ? `<img src="cid:supplypulse-logo" alt="SupplyPulse" width="56" height="56" style="display:block;margin:0 auto 14px auto;border-radius:12px;border:1px solid ${T.line};">` : ''}

            <div style="font-size:22px;font-weight:800;color:${T.text};letter-spacing:-0.3px;">
              Supply<span style="color:${T.pulse};">Pulse</span>
            </div>
        ...
        <tr>
          <td style="padding:24px 32px 0 32px;">
            <h1 style="margin:0;font-size:18px;font-weight:700;color:${T.text};line-height:1.4;">${title}</h1>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:20px 32px;text-align:center;">
            <div style="font-size:13px;font-weight:700;color:${T.text2};">
              Supply<span style="color:${T.pulse};">Pulse</span>
            </div>
            <div style="font-size:11px;color:${T.text3};margin-top:6px;">
              ${footerNote || 'This message was sent automatically by the SupplyPulse autonomous agent. No human intervention.'}
            </div>
            <div style="font-size:10px;color:${T.line};margin-top:8px;">DistCo Karachi North Operations &mdash; ${new Date().getFullYear()}</div>
          </td>
        </tr>
...
async function sendEmail({ to, subject, body, badge, footerNote, htmlOverride }) {
  ...
  const from = process.env.SMTP_FROM || process.env.SMTP_USER || 'noreply@supplypulse.ai';
